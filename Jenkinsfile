        ...
        stage('git checkout') {
            steps {
                // Get some code from a GitHub repository
                 git branch: 'main', url: 'https://github.com/Auraofbee/jjtech-maven-sonarqube-nexus-prometheus-project.git'
            }
        ...
        // }
          ...
          stage('Upload artifact to Nexus') {
            steps {
                ...
                sh "sed -i \"s/.*<username><\\/username>/<username>$USER_NAME<\\/username>/g\" ${WORKSPACE}/settings.xml"
                sh "sed -i \"s/.*<password><\\/password>/<password>$PASSWORD<\\/password>/g\" ${WORKSPACE}/settings.xml"
                sh "sed -i 's|http://35.89.37.232:8081/repository/maven-snapshots/|http://35.89.37.232:8081/repository/maven-snapshots/|g' ${WORKSPACE}/settings.xml"
                sh "sed -i 's|http://35.89.37.232:8081/repository/maven-releases/|http://35.89.37.232:8081/repository/maven-releases/|g' ${WORKSPACE}/settings.xml"
                sh "sed -i 's|<sonar.host.url>http://172.31.31.199:9000</sonar.host.url>|<sonar.host.url>http://172.31.31.199:9000</sonar.host.url>|g' ${WORKSPACE}/settings.xml"
                sh 'sudo cp ${WORKSPACE}/settings.xml /var/lib/jenkins/.m2'
                dir('JavaWebApp/') {
                sh "sed -i 's|http://35.89.37.232:8081/repository/maven-snapshots/|http://35.89.37.232:8081/repository/maven-snapshots/|g' pom.xml"
                sh "sed -i 's|http://35.89.37.232:8081/repository/maven-releases/|http://35.89.37.232:8081/repository/maven-releases/|g' pom.xml"                
                sh 'mvn clean deploy -DskipTests'
                }
              }
